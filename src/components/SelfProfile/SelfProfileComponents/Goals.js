...
import AddGoalButton from "../SelfProfileComponents/AddGoalButton.jsx"
...
export default function Goals(props) {
    ...
    const newGoal =
        <div className="skill skill--badge">
            <label htmlFor="newGoalSubject" className="form__label">New subject</label>
            <input type="text"
                name="newGoalSubject"
            ...
            <label htmlFor="newGoalComment" className="form__label">Comment</label>
            <input type="text"
                name="newGoalComment"
                className={"form__input-suffix-container"} 
    ...
    const goalsRowItems = wantsToLearnItems.map((goal, index) =>
        (<li key={goal.id} className={index + 1 === numberOfGoals ? "goals__row goals__row--last" : "goals__row"}>
            <div class="goal">
...
{       
        ...
        <li className={index + 1 === numberOfGoals? "goals__row goals__row--last" : "goals__row"}>  
            <div className="skill skill--badge">
                ...
                <input type="text"
                    ...
                    onChange={props.handleChangeArrays}/>
<button type="button" class="button" name="delete_goal" id={index} onClick={props.handleButtonClick} style={{padding: '10px'}} ><Delete /></button>
            </div>
        </li>)
}
    </div>
    ...
    return (
        <div>
            ...
            <ul className="goals">
                {props.dataShared.isEditModeDisabled ? goalsRowItems : goalsRowItemsEdit}
            </ul>

            {!props.dataShared.isEditModeDisabled && 
            <div> 
                {newGoal}
                <AddGoalButton name = "create_goal" onClick={props.handleButtonClick} handleButtonClick={props.handleButtonClick}/>
                {editBtnsGoals} 
            </div>}
        </div>
    )
